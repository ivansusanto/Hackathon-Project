      ...
      http.post("users/login", data).then((res) => {
        if(res.data.data.role == 0){
          if(res.data.message != "berhasil login"){
            ...
            window.location.replace("/admin/master-user")
          }
        ...
        <div className="relative flex z-10 justify-center">
          <div className=" flex items-center justify-center h-screen">
            <div className="border-solid border-2 rounded-lg py-12 px-28 bg-white" style={{height: "500px", width: "1000px"}}>
              ...
              <div>
                <button className="btn btn-primary w-full mt-12 mb-1 text-xl font-bold" onClick={submitHandler} disabled={!filled}>Masuk</button>
              </div>
            </div>
          </div>
        </div>
